...
#define STUFF_LIST { 21, 22, 23, 1 }
#define LEDS_MASK2 ( (1<<21) | (1<<22) | (1<<23) | (1<<1) )
...
void open_garage(){
	...
	const uint8_t leds_list[LEDS_NUMBER+1] = STUFF_LIST;

	// Configure LED-pins as outputs.
	LEDS_CONFIGURE(LEDS_MASK2);
 ...
 * @param[in] p_data   Data received over NUS
...
static void nus_data_handler(ble_nus_t * p_nus, uint8_t * p_data, uint16_t length)
{
	...
	if(data_array[0] == 'o' &&
		...
		err_code = ble_nus_string_send(&m_nus, data_array, i);
		if(err_code){
			//...	
		}
